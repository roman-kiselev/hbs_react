...
import SectionService from "../../service/objects/SectionService.js";
...
class SectionsController {
    async createSections(req, res) {
        try {
            ...
            const sections = await Models.Section.create({
                number,
    ...
    async getOneSections(req, res) {
        try {
            const { id } = req.params;
            const sections = await Models.Section.findOne({
                where: { id },
                include: { all: true },
            });
            return res.json({ sections });
    ...
    async getUniqueSection(req, res) {
        try {
            const { id: objectBuildId } = req.params;

            const data = await SectionService.getUniqueSections(objectBuildId);
