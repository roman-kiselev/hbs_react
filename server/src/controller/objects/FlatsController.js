...
class FlatsController {
    ...
    async createFlats (req, res) {
        try {
            ...
            const flat = await Models.Flats.create({number, objectBuildId: idObject})
    ...
    async getAllFlats (req, res) {
        try {
            ...
            const flats = await Models.Flats.findAll({include: {all: true}})
    ...
    async getOneFlats (req, res) {
        try {
            const {id} = req.params
            const flat = await Models.Flats.findOne({where: {id},include: {all: true}})
            return res.json({flat})
