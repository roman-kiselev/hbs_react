...
import { HeadersHeatConfig, createHeatTemplate } from "../../config";

import { MainAddMeter } from "../../models";
import db from "../../models/db";
import { getHeatMetersByNumberFlat } from "../../services";

class MainTableHeatController {
    ...
    async addNewMeter(req, res) {
        try {
            ...
            const heatMeter = await MainAddMeter.create({
                section: section,
                floor: floor,
                flat: flat ? flat : 0,
    ...
    async getAllHeatMeters(req, res) {
        try {
            ...
            const heatMeters = await MainAddMeter.findAndCountAll({
                where: {
    ...
    async editHeatMeterById(req, res) {
        try {
            ...
            const heatMeter = await MainAddMeter.findByPk(id);
    ...
    async getTemplateHeat(req, res) {
        try {
            ...
            // Там функция принимает на вход квартиру, номер счётчика, секцию, этаж, данные из query
            const { objectBuildId, line, template } = req.query;
            const metersAll = await MainAddMeter.findAll({
                where: {
                    objectBuildId,
                    typeMeter: "Счётчик тепла",
                    line,
            ...
            const x = await MainAddMeter.findAll({
                where: {
                    objectBuildId,
                    typeMeter: "Счётчик тепла",
                    line,
            ...
            // Получаем максимальное значение квартиры
            const maxFlat = await MainAddMeter.findOne({
                where: {
                    objectBuildId,
                    typeMeter: "Счётчик тепла",
                    line,
        ...
        try {
            ...
            for (const meter of data) {
                ...
                if (meter.idDb) {
                    ...
                    const [meterAdd, created] = await MainAddMeter.findOrCreate(
                        {
                            where: {
                                flat,
                                numberMeter,
...
export default new MainTableHeatController();
