...
import { HeadersElectricalConfig, createHeatTemplate } from "../../config";
import { MainAddMeter } from "../../models";
import db from "../../models/db";
import { getElectricalMetersByNumberFlat } from "../../services";

class MainTableElectricalController {
    async createMeter(req, res) {
        try {
            ...
            const newMeter = await MainAddMeter.create({
                section,
                floor,
                flat: flat ? flat : 0,
    ...
    async editMeterById(req, res) {
        try {
            ...
            const meter = await MainAddMeter.findByPk(id);
...
export default new MainTableElectricalController();
